...
- `server/api.ts` — `OpenAPIHono` routes; every route declares a zod schema (that's where the OpenAPI spec comes from). **Default deny**: every method+path outside `PUBLIC_OPERATIONS` (`GET /health`, `GET /doc`) requires a principal, so a new route is authenticated unless you say otherwise. CSRF is a same-origin check on every unsafe method, applied to session callers only — not `hono/csrf`, which is a no-op on `application/json` (#15). `/me` needs a principal; `/tokens*` needs an interactive session. **Ends in a terminal `all("*")` returning JSON, which must stay last** — an authenticated unknown path clears the deny guard and matches nothing, and the adapter mounts React Router after this app, so without it the request reached the browser splat and an API client got an HTML page (unlike `app/routes.ts`, order really is load-bearing here)
- `server/api-organization.ts` — the `/api/v1/organization/*` sub-app, mounted into `apiApp` **above** its terminal `all("*")`. Registered with full paths and mounted at `/` rather than under a prefix, so what `/doc` advertises is the string written beside each route — `check:docs-sync` matches those literals against the README. Four things are load-bearing. **The tenant is the credential's**, never a parameter, and is re-checked against `member` on every request (a session outlives a removal; a token's organization is stamped once at creation). **Writes are session-only** via `requireInteractivePrincipal`, because a token that can promote its own owner survives its own revocation. **Every write target is resolved through an org-scoped read first and 404s when absent** — better-auth resolves member and invitation ids _globally_ and compares organizations afterwards, so `remove-member` runs its last-owner check first and will tell an outsider whether an id they do not own belongs to an owner; missing and other-tenant have to be one answer. And **`auth.api.*` bypasses the rate limiter**, so each write charges the binding itself under the better-auth path it delegates to — one budget per address across both doors, with `rateLimitClassFor` deciding the class rather than a second table. `can()` here picks which refusal is heard first; `ORGANIZATION_ROLES` in `@starter/auth` is what enforces
...
- `app/entry.server.tsx` — `handleError` reports loader/action failures; the stream `onError` reports SSR render failures (Sentry dedupes the overlap). Expected 4xx `ErrorResponse`s — a loader or action throwing `data()` below 500, the router's own 405 for a POST at a route with no action — are logged at `warn` and never reach Sentry, mirroring `observabilityErrorHandler`; deny-path tests in `app/__tests__/entry-server.test.ts`. **Unmatched URLs are not in that set**: the `*` splat renders the branded 404 from a _successful_ loader, so a crawler probing `/wp-admin` is a normal render, not a route error
- `app/routes.ts` — explicit route config (NOT file-based routing). The `*` splat is written last for readability only — matching is by specificity, not declaration order, so a route added below it is still reachable (the comment there has the mechanism)
...
- **`/accept-invitation` is the one loader that deliberately does not call
  `requireUser`.** The link arrives in a mailbox, so the reader may have no
  session — and may have no account at all — and redirecting to `/login` would
  lose the invitation. What makes that safe is that the signed-out branch
  returns nothing the caller did not already supply: it echoes the id back and
  stops, with no organization name, no inviter, and no confirmation that the id
  is real. Everything past it comes from `auth.api.getInvitation`, which runs
  better-auth's own recipient check. Do not "fix" this by adding a guard; do
  not copy the exemption into any other loader. Accepting is a **button, not
  the loader** — a mutating GET is spent by a link prefetch or a mail scanner
  before the reader sees it, and an invitation is single-use. Three facts about
  better-auth 1.6.26 are load-bearing, read from
  `plugins/organization/routes/crud-invites.mjs`: it **generates no invitation
  URL** (`@starter/auth/invitation` owns the path, and `auth-redirects.ts`
  re-exports it); accept **sets the active organization itself**, so calling
  `setActive` after it is redundant, exactly as with create; and expired,
  revoked and already-accepted are **indistinguishable**, collapsed into one
  `400`, while `getInvitation`'s version of that refusal carries **no error
  code at all** — so `invitation-state.ts` maps the code first and falls back
  to the status, the reverse of `reset-password-errors.ts`, and both halves are
  needed. The signed-out round trip is `?invitation=<id>` on `/login` and
  `/register`, deliberately **not** a general `redirectTo`: it carries an id and
  the only path it can produce is one `auth-redirects.ts` owns, so there is no
  open redirect to get wrong. `/register` threads it into the verification
  `callbackURL` — the `POST_VERIFICATION_REDIRECT` trap again — and
  `VerificationNotice` takes it as a prop so a **resend** mints the same
  destination the first link carried
