...
/**
 * Orientation styling must be written as `data-[orientation=horizontal]:`, never
 * the `data-horizontal:` shorthand.
 *
 * Tailwind has no such shorthand, so it emits **no rule at all** — the class is
 * silently dropped rather than failing. That is what shipped here: the root kept
 * its default `flex-direction: row`, so `TabsList` and `TabsContent` sat side by
 * side, the content track collapsed to 0px, and any wide child (a `<pre>`) blew
 * out of it and gave the whole page a horizontal scrollbar.
 *
 * `separator.tsx` still carries the same dead shorthand.
 */
function Tabs({
  className,
